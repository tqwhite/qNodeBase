...
var moduleFunction = function(qtools) {
	...
	const findGoodConfigPath=(pathParm, workingDirectory, resolve)=>{
		...
		const result=workingDirectory!='/'?path.join(workingDirectory, pathParm):''
			if (resolve){
	...
	qtools.configFileProcessor.getConfig = (configPath, workingDirectory, resolve=false) => {
		const validConfigPath=findGoodConfigPath(configPath, workingDirectory, resolve);
		if (!validConfigPath){
