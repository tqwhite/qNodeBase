...
var moduleFunction = function(self) {
  ...
  self.removeNullElements = function(target, removeFalsy, keepFunction) {
    removeFalsy = !(typeof removeFalsy == "undefined" || removeFalsy === "" ||
      removeFalsy === false);
  ...
  self.byObjectProperty = function(fieldName, transformer) {
    ...
    if (type == "object" || type == "arguments") {
      ...
      for (var i in inObject) {
        ...
        var showElement;
        if (
          [ "object", "array", "function", 'map' ].indexOf(qtools.toType(element)) ==
            -1
        ) {
          if (element == null) {
            ...
            if (element.length > maxShowStringLength) {
              element = element.substr(0, maxShowStringLength) +
                " ... (length=" +
                element.length +
                ")";
            }
          ...
          if (toString) {
            var tmp = "item #" + count + " named '" + i + "' is a " +
              qtools.toType(element) +
              " value= " +
              showElement;
          } else {
            var tmp = ("item #" + count + " named '").grey + i.red +
              "' is a ".grey +
      ...
      for (var i = 0, len = list.length; i < len; i++) {
        ...
        if (!toString) {
          ...
          outString += "item #" + count + " index '" + i + " is " +
            qtools.toType(element) +
            "\n";
        }
        count++;
    ...
    inObject.forEach((element, i, all)=>{
      ...
      for (var i = 0, len = outList.length; i < len; i++) {
        ...
        if (!toString) {
        	var message="qtools.listProperties says, input was of not an object or array. It was type '"+qtools.toType(inObject)+"'";
  ...
  self.clone = function(inObj, func) {
    let convertFunction = typeof func == "function" ? func : function(inData) {
        return inData;
      };

    if (
      [ "string", "number", "boolean", "undefined" ].indexOf(typeof inObj) >
        -1 ||
        inObj === null
    ) {
  ...
  self.snakeToCamel = function(inString, upperCaseInitial) {
    ...
    self.localReplaceObject["leaveUnmatchedTagsIntact"] = leaveUnmatchedTagsIntact
  ...
  self.strToBool = function(inData) {
    ...
    const dateString = dateValues.year + "." + dateValues.month + "." +
      dateValues.day +
      "." +
      dateValues.hours +
      "." +
