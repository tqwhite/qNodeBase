...
var moduleFunction = function(self) {
	...
	var sampleArg={
		...
		args = args || {};
		args.iterations = 10;
		args.keyLength = Math.max(Math.ceil(args.resultLength / 2), 2) || 3; //returns hex, two chars per result byte

		
		if (typeof (args.allowRepeat) != 'undefined'){
			if (args.inString){
				args.allowRepeat=true;
			}
			else{
				args.allowRepeat=false;
			}
		}
		else{
			if (args.inString && !args.allowRepeat){
				throw "qtools.newId() says, can't specify inString and allowRepeat===false"
			}
		}
		
		if (args.uniqueSpaceId && args.allowRepeat){
			throw "qtools.newId() says, can't have uniqueSpaceId and allowRepeat===true"
		}
	...
	self.templateReplace = function(args) {
		...
		outString = outString.replace(/<!([a-zA-Z0-9]+)!>/g, self.evaluatorFunction);
