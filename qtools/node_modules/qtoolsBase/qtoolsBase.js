...
var moduleFunction = function(self) {
	...
	self.getByProperty = function(inData, propertyName, propertyValue, defaultValue) {
...
// 				}
	...
	self.templateReplace = function(args) {
		...
		// }
		var template = args.template?args.template:"MISSING TEMPLATE IN qtools.templateReplace()";
...
// 		);
	...
	self.parseCommandLine = function(args) {
		...
		var valuesSplitCharacter = args && args.valuesSplitCharacter ? args.valuesSplitCharacter : '';
		...
		*/
		...
		return {
			calledFromCommandLine: process.env.SHLVL==1,
			fileList: fileList,
	...
	self.convertNumericObjectToArray = function(inObj, removeMissingElements=true, defaultValue) {
	...
	const shortTimeStamp=`${today.toLocaleDateString().replace(/(-|\/)/g, '').replace(/^20/,'')}${secondsToday}`;
...
self.tsvToJsObjectList=(inString, numberPrefixFieldNames=false)=>{
	...
	let numberPrefix=0;
		...
		const propertyName=numberPrefixFieldNames?`${numberPrefix++}_${fieldNames[inx]}`:fieldNames[inx];
