...
var moduleFunction = function(self) {
	...
	self.message = function(messageData, color, longFilename) {
		...
		var prefix = '\n**==*: ';
		var showEmployer = this.employerFilename ? '(' + this.employerFilename + ')' : '';
		var suffix = longFilename ? ' (' + __dirname + '/' + this.employerFilename + ')\n' : showEmployer + '\n';
		var message;
	...
	self.byObjectProperty = function(fieldName, transformer) {
		...
		return fullNameSort = function(a, b) {
			...
			var localTransformer = transformer; //for debug
	...
	self.validateProperties = function(args, doNotDie) {
		...
		*/
		...
		for (var i = 0, len = propList.length; i < len; i++) {
			...
			var requiredType = propList[i].requiredType ? propList[i].requiredType : false;
