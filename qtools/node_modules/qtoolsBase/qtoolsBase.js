...
var moduleFunction = function(self) {
	...
	self.dump = function(inData, returnStringFlag) {
		...
		if (returnStringFlag) {
			return (
				util.inspect(inData, { depth: null, maxArrayLength: null })
			);
		} else {
			console.log(
	...
	self.templateReplace = function(args) {
		...
		self.localReplaceObject['indexNumber'] = args.indexNumber
			? args.indexNumber
			: 0;
		self.localReplaceObject.useSimplePath=(args.useSimplePath===true)?true:false;
	...
	self.getDateString = function(format, dateObj) {
		...
		qtools.logWarn(`NOTE: getDateString() is deprecated, use qtools.dayjs() instead. Eg, dayjs().format('YYYY-MM-DD')`);
	...
	self.flattenObject = function(inObj, options={}) {
		const includeIntermediates=options.includeIntermediates?options.includeIntermediates:false;
